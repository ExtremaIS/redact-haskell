...
ifeq ($(MODE), cabal)
> @install -m 0755 \
>   "$(shell cabal list-bin $(CABAL_ARGS) redact)" \
>   "$(bindir)/redact"
else
...
> @mkdir -p "$(man1dir)"
> @install -m 0644 -T <(gzip -c doc/redact.1) "$(man1dir)/redact.1.gz"
.PHONY: install-man
...
> @pandoc -s -t man -o doc/redact.1 \
>   --variable header="redact Manual" \
